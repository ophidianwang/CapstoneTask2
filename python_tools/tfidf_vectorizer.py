...
def sim_matrix():
    ...
    with open('tfidf_result/vectorize_temp.txt', 'w') as f:
    ...
    with open('tfidf_result/cuisine_sim_matrix.csv', 'w') as f:
    ...
    with open('tfidf_result/cuisine_indices.txt', 'w') as f:
        f.write("\n".join(c_names))

    # create json file for d3.js
    output = []
    cursor = 0
    while cursor < len(c_names):
        single_matrix = []
    ...
    with open('tfidf_result/result.json', 'w') as f:
